...
*   **Strategy:** Runs the same library helpers against the *real* UI and *real* data structures. We will use a "Verification Bridge" (a small module that *only* adds the `FP_VERIFY` logging to a real system) to maintain a consistent assertion API across both tiers.
...
async function verifyResult(page: Page, key: string, predicate: (data: any) => boolean) {
  await page.waitForFunction(({ key, predicateStr }) => {
  ...
  }, { key, predicateStr: predicate.toString() }, { timeout: 5000 });
}
```
