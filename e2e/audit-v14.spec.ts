...
test('V14 Setup Audit', async ({ page }) => {
  ...
  const setupAudit = await page.evaluate(() => {
    return {
        tabs: Array.from(document.querySelectorAll('[data-tab], .tabs .item, [data-application-part]')).map(el => ({
            tag: el.tagName,
      ...
      const createAudit = await page.evaluate(() => {
        return {
            ...
            packages: Array.from(document.querySelectorAll('.package')).map(el => ({
                ...
                title: el.querySelector('.package-title')?.textContent?.trim() || el.querySelector('h3')?.textContent?.trim(),
