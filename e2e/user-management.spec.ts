...
test.describe('User Management Verification', () => {
  ...
  test('user management: create, update, and login', async ({ browser, page, foundry }) => {
    ...
    const actorId = await page.evaluate((name) => (window as any).game.actors.getName(name).id, actorName);
    const userId = await page.evaluate((name) => (window as any).game.users.getName(name).id, testUserName);
    
    await foundry.state.assignActorToUser(userId, actorId);

    // Verify assignment
    const assignedActorId = await page.evaluate((uId) => (window as any).game.users.get(uId).character?.id, userId);
