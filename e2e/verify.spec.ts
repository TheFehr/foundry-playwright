...
test.describe('Library Verification Suite', () => {
  ...
  test('foundry.state: document management', async ({ page, foundry }) => {
    ...
    await verifyResult(page, 'actor-create', (data: any, extra: any) => data.name === extra.actorName, { actorName });
    await foundry.state.grantCurrency(actorName, 100, 'gp');
    await verifyResult(page, 'actor-update', (data: any, extra: any) => {
  ...
  test('foundry.state: settings management', async ({ page, foundry }) => {
    ...
    const val = await page.evaluate(() => (window as any).game.settings.get('fake-module', 'test-string'));
  ...
  test('foundry.helpers: tour suppression', async ({ page, foundry }) => {
    await page.evaluate(() => {
  ...
  test('foundry.ui: drop simulation', async ({ page, foundry }) => {
    ...
    await expect(async () => {
        const logs = await page.evaluate(() => (window as any).FP_VERIFY?.logs['actor-sheet-drop'] || []);
