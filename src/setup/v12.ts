...
/**
 * Setup adapter for Foundry VTT Version 12.
 *
 * This holds the full Application V1 setup-screen implementation: the
 * baseline behavior for the UI generation V12 introduced, which V13 mostly
 * kept unchanged (V14SetupAdapter is the real rewrite, onto ApplicationV2).
 * V13SetupAdapter extends this and overrides only the couple of places it
 * actually diverges - see its class doc. Newer/older sibling generations
...
export class V12SetupAdapter implements SetupAdapter {
   ...
   * ghcr.io/felddy/foundryvtt:12.343.0. V13 prefixed these
   * ("setup-packages-systems") - overridden in V13SetupAdapter.
   */
  ...
  /**
   * The package-name filter input inside an install-package dialog. V12's
   * `input[name="filter"]` has no accessible name at all (no aria-label, no
   * associated label text) - confirmed live against
   * ghcr.io/felddy/foundryvtt:12.343.0. V13 gave the same input an
   * accessible name of "Filter" - overridden in V13SetupAdapter to use
   * role/name there instead of targeting the input directly.
   */
  protected filterBoxLocator(dialog: Locator): Locator {
    return dialog.locator('input[name="filter"]');
  }
  ...
  async switchTab(page: FoundryPage, tabName: string): Promise<void> {
    ...
    const tabLocator = page
      .locator(`nav h2, .navigation h2, h2, #${modulesSectionId} h2`)
    ...
    await page.waitForFunction(
      ...
      this.packagesSectionId(dataTab),
      { timeout: 10000 },
    );
  ...
  async installSystem(page: FoundryPage, systemId: string, _systemLabel: string): Promise<void> {
    ...
    const filterBox = this.filterBoxLocator(installDialog);
  ...
  async installSystemFromManifest(page: FoundryPage, manifestUrl: string): Promise<void> {
    ...
    const systemsSectionId = this.packagesSectionId("systems");
    const verificationSelector = systemId
      ? `#${systemsSectionId} [data-package-id="${systemId}"]`
      : `#${systemsSectionId} [data-package-id]`;
  ...
  async createWorld(
    ...
    // :not([disabled]) excludes an auto-derived, read-only "path" preview
    // field rendered alongside the actual editable id field - confirmed on
    // V12: a disabled `input[name="path"]` sorts before the real
    // `input[name="id"]` in DOM order, so an undiscriminated selector list
    // matches the wrong one via .first().
    const pathInput = createDialog.locator(
      'input[name="id"]:not([disabled]), input[name="name"]:not([disabled]), ' +
        'input[name*="path" i]:not([disabled]), input[placeholder*="Path" i]:not([disabled])',
    );
  ...
  async launchWorld(page: FoundryPage, worldId: string): Promise<void> {
    ...
    const launchBtn = worldBox
      ...
      .first();
    // The launch button is hidden by default (shown on hover); wait for it to be
...
/**
 * Game adapter for Foundry VTT Version 12.
 */
export class V12GameAdapter extends BaseGameAdapter {
  version = 12;
