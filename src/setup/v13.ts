...
export class V13SetupAdapter implements SetupAdapter {
  ...
  async installSystem(page: Page, systemId: string, systemLabel: string): Promise<void> {
    ...
    const installBtn = page.locator('button, [role="button"]').filter({ hasText: /Install/i }).filter({ hasText: /System/i }).first();
    await expect(installBtn).toBeVisible({ timeout: 10000 });
    await installBtn.evaluate(el => (el as HTMLElement).click());
    
    const installDialog = page.locator("dialog, .application, .window-app, foundry-app, section.window-app").filter({ hasText: /Install System/i }).last();
    ...
    const installButton = packageRow.locator('button[data-action="installPackage"], button:has-text("Install")').first();
  ...
  async installModules(page: Page, moduleIds: string[]): Promise<void> {
    ...
    for (const modId of moduleIds) {
      const moduleBox = page.locator(`[data-package-id="${modId}"], [data-module-id="${modId}"]`).first();
      if (await moduleBox.count() === 0 || await moduleBox.isHidden()) {
        const installBtn = page.locator('button, [role="button"]').filter({ hasText: /Install/i }).filter({ hasText: /Module/i }).first();
        await expect(installBtn).toBeVisible({ timeout: 10000 });
        await installBtn.evaluate(el => (el as HTMLElement).click());
        
        const installDialog = page.locator("dialog, .application, .window-app, foundry-app, section.window-app").filter({ hasText: /Install Module/i }).last();
        await expect(installDialog).toBeVisible({ timeout: 20000 });
  ...
  async createWorld(page: Page, worldId: string, systemLabel: string, systemId: string): Promise<void> {
    ...
    const createBtn = page.locator('button').filter({ hasText: /Create World/i }).first();
    await createBtn.evaluate(el => (el as HTMLElement).click());
    
    // Target the specific world-config form
    const createDialog = page.locator("form#world-config, dialog, .application").filter({ hasText: /World|Create/i }).last();
    ...
    const submitBtn = createDialog.locator('button[type="submit"], button').filter({ hasText: /Create World|Create/i }).first();
  ...
  async deleteWorldIfExists(page: Page, worldId: string): Promise<void> {
    ...
    if ((await worldBox.count()) === 1) {
      ...
      const dialog = page.locator("dialog,div,section,form").filter({ hasText: new RegExp(`Delete World: ${worldId}`, 'i') }).last();
