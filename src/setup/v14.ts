...
export class V14SetupAdapter implements SetupAdapter {
  ...
  async handleEULA(page: Page): Promise<void> {
    ...
    const analyticsDialog = page
      .locator("dialog, foundry-app, .application")
      .filter({
    ...
    if (page.url().includes("/license") || (await eulaHeading.count()) > 0) {
      ...
      if ((await checkbox.count()) > 0) {
        ...
        if (!(await checkbox.isChecked())) {
          console.warn(
            "[V14SetupAdapter] Checkbox still not checked after click. Retrying with evaluate...",
          );
      ...
      if ((await agreementBtn.count()) > 0 && (await agreementBtn.isVisible())) {
        ...
        await agreementBtn
          .click({ timeout: 5000 })
          .catch(() => agreementBtn.click({ force: true }));
        ...
        try {
          ...
          console.warn(
            "[V14SetupAdapter] Timeout waiting for EULA navigation. Retrying with evaluate click...",
          );
          await agreementBtn.evaluate((el) => (el as HTMLElement).click());
          await page
            .waitForURL((u) => !u.pathname.includes("/license"), { timeout: 10000 })
            .catch(() => {
              throw new Error(
                "[V14SetupAdapter] Failed to navigate away from EULA screen after clicking Agree.",
              );
            });
        }
  ...
  async installSystem(page: Page, systemId: string, _systemLabel: string): Promise<void> {
    ...
    const installButton = packageRow
      .locator('button[data-action="installPackage"], button:has-text("Installed")')
      .first();
    await expect(installButton).toBeVisible({ timeout: 10000 });

    const isInstalled =
      (await installButton.getAttribute("disabled")) !== null ||
      (await installButton.innerText()).includes("Installed");
  ...
  async createWorld(
    ...
    if (!creationStarted && !page.url().includes("/create")) {
      throw new Error(
        "[V14SetupAdapter] Failed to navigate to world creation screen after multiple attempts.",
      );
    }
  ...
  async deleteWorldIfExists(page: Page, worldId: string): Promise<void> {
    ...
    if ((await worldBox.count()) === 1 && (await worldBox.isVisible())) {
      ...
      const dialog = page
        .locator("dialog, foundry-app, .application")
        .filter({ has: page.locator("#delete-confirm") })
        .last();
